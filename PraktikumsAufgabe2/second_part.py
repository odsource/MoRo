...
def followLine(robot, myWorld, p, q):
    p1, p2 = p
    q1, q2 = q
...
def gotoGlobal(robot, myWorld, v, p, tol):
    p1, p2 = p
...
def followPolyline(robot, myWorld, v, line):
    tol = 1
    for point in line:
        if point == line[-1]:
            tol = 0.1
        gotoGlobal(robot, myWorld, v, point, tol)
...
ln = [[4, 10], [13.0, 10]]
pln = [[2, 18], [2, 7], [4, 10], [13.0, 10], [13, 2]]
myWorld.drawPolyline(ln)
#myRobot.setNoise(0, 0, 0)
myWorld.setRobot(myRobot, [1, 18, 0])

#followLine(myRobot, myWorld, ln[0], ln[1])

#gotoGlobal(myRobot, myWorld, 1, [13, 10], 0.1)
followPolyline(myRobot, myWorld, 0.5, pln)
myWorld.close()